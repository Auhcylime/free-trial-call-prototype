...
import { Check, Shield, Lock, Wifi, ChevronDown, Download } from "lucide-react";
...
function getAgeMonths(dob: { mm: string; dd: string; yyyy: string }): number {
  if (!dob.yyyy || !dob.mm) return 18;
  const birthDate = new Date(parseInt(dob.yyyy), parseInt(dob.mm) - 1, parseInt(dob.dd) || 1);
  const now = new Date(2026, 1, 26);
  return (now.getFullYear() - birthDate.getFullYear()) * 12 + (now.getMonth() - birthDate.getMonth());
}

function getCaseNotes(catName: string, breed: string | null): { note: string; cta: string; date: string }[] {
  const breedNote = (() => {
    if (breed === "Persian cat" || breed === "Exotic Shorthair") {
      return { note: `The vet noticed ${catName}'s eyes were a bit watery and recommended switching to a dust-free litter and cleaning around the eyes daily for the next week.`, cta: `Follow up on ${catName}'s eyes` };
    }
    if (breed === "Maine Coon" || breed === "Norwegian Forest") {
      return { note: `The vet recommended switching ${catName} to a high-protein dry food and monitoring her weight over the next 2 weeks.`, cta: `Follow up on ${catName}'s diet` };
    }
    if (breed === "Siamese cat" || breed === "Bengal") {
      return { note: `The vet said ${catName}'s occasional sneezing is likely environmental and suggested keeping windows closed during high-pollen days.`, cta: `Follow up on ${catName}'s sneezing` };
    }
    if (breed === "British Shorthair" || breed === "Burmese") {
      return { note: `The vet advised reducing ${catName}'s portion size by 10% and adding a 15-minute play session each evening to help with weight.`, cta: `Follow up on ${catName}'s weight` };
    }
    if (breed === "Sphynx cat") {
      return { note: `The vet recommended bathing ${catName} weekly with a gentle cat shampoo and checking for any skin irritation between baths.`, cta: `Follow up on ${catName}'s skin` };
    }
    if (breed === "Scottish Fold") {
      return { note: `The vet noticed ${catName} was a little stiff after resting and suggested a joint supplement to start this week.`, cta: `Follow up on ${catName}'s joints` };
    }
    if (breed === "Ragdoll" || breed === "Himalayan") {
      return { note: `The vet said ${catName}'s coat is looking great but recommended a daily brush to prevent matting, especially around the belly.`, cta: `Follow up on ${catName}'s grooming` };
    }
    return { note: `The vet recommended switching ${catName} to a high-protein dry food and monitoring her weight over the next 2 weeks.`, cta: `Follow up on ${catName}'s diet` };
  })();

  return [
    { ...breedNote, date: "Feb 27, 2026" },
    { note: `The vet checked ${catName}'s teeth and noted mild tartar buildup. Recommended dental treats and a follow-up cleaning in 3 months.`, cta: `Follow up on ${catName}'s dental`, date: "Feb 14, 2026" },
    { note: `The vet confirmed ${catName} is up to date on all vaccinations. Next booster due in 12 months.`, cta: `Follow up on ${catName}'s vaccinations`, date: "Jan 30, 2026" },
  ];
}
...
export function PostCallScreen({ onNext, catProfiles, setCatProfiles, userName, onArticleTap, openAirvetDrawer, onAirvetDrawerOpened }: PostCallScreenProps) {
  ...
  const caseNotes = getCaseNotes(catName, activeCat.breed);
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="px-5 pt-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-white/60 text-[12px] font-semibold uppercase tracking-wide">Airvet case notes · {caseNote.date}</p>
              ...
              {[
                ...
                <div key={b} className="flex items-center gap-3">
                  <svg width="20" height="20" viewBox="0 0 20 20" className="flex-shrink-0">
                    ...
                    <circle cx="10" cy="10" r="10" fill="#10b981" mask="url(#checkCutPost)" />
                  </svg>
