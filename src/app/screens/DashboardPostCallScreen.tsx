...
// Breed-specific advice (reused from PreCallScreen)
...
function getAdvice(breed: string | null): { tip: string; cta: string } {
  if (breed && BREED_ADVICE[breed]) return BREED_ADVICE[breed];
  return GENERIC_ADVICE[0];
}

interface DashboardPostCallScreenProps {
  onNext: () => void;
...
export function DashboardPostCallScreen({ onNext, catProfiles, setCatProfiles, userName, onArticleTap, openAirvetDrawer, onAirvetDrawerOpened }: DashboardPostCallScreenProps) {
  ...
  const advice = getAdvice(activeCat.breed);
